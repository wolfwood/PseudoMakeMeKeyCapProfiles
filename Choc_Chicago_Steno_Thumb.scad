	...
	translate([0,0,-10])cube([40,40,20], center=true);
      ...
      if(Stem == true){
        rotate([0,0,StemRot]){
          ...
          translate([0,0,-.001])skin([for (i=[0:stemLayers-1 +20]) transform(translation(StemTranslation(i,keyID)), rounded_rectangle_profile(StemTransform(i, keyID),fn=fn,r=1 /*StemRadius(i, keyID) */ ))]); //outer shell
       }
