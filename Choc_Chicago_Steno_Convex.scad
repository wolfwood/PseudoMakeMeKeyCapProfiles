...
module keycap(keyID = 0, cutLen = 0, visualizeDish = false, crossSection = false, Dish = true, Stem = false,StemRot = 0, homeDot = false, Stab = 0, Legends = false) {
  ...
  FrontCurve = [ for(i=[0:frontmost]) transform(FrontPath[frontmost - i], DishShape(DishDepth(keyID), FrontDishArc(frontmost - i), DishDepth(keyID)+1.5, d = 0)) ];
  backmost = len(BackPath)-1;
  BackCurve  = [ for(i=[0:backmost])  transform(BackPath[backmost - i],  DishShape(DishDepth(keyID),  BackDishArc(backmost - i), DishDepth(keyID)+1.5, d = 0)) ];
